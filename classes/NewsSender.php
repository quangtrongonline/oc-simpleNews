<?php namespace Quangtrong\News\Classes;
...
class NewsSender
{
     ...
     */
    protected $templateNamespace = 'quangtrong.news::mail.email_';
    ...
    protected function template($locale)
    {
        ...
        foreach ($langs as $lang) {
            if (File::exists(base_path().'/plugins/quangtrong/news/views/mail/email_'.$lang.'.htm')) {
    ...
    protected function sendToActiveSubscribers()
    {
        ...
        if (Categories::count() > 0) {
            $ids[] = 0;
            $relations = Db::table('quangtrong_news_relations');
    ...
    protected function send($receiver)
    {
        ...
        if ($this->queued) {
            $qId = Queue::push('\Quangtrong\News\Classes\SendNews', [
                'template' => $template,
                'params'   => $params,
                'receiver' => $receiver,

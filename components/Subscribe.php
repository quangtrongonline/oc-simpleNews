...
class Subscribe extends ComponentBase
{
    ...
    {
        return [
            'name'        => 'quangtrong.news::lang.component.subscribe',
            'description' => ''
        ];
    }
    ...
    public function onRun()
    {
        ...
        $this->page['text_messages'] = Lang::get('quangtrong.news::lang.messages.subscribed');
        $this->page['text_name']     = Lang::get('quangtrong.news::lang.form.name');
        $this->page['text_email']    = Lang::get('quangtrong.news::lang.form.email');
        $this->page['text_category'] = Lang::get('quangtrong.news::lang.form.category');
        $this->page['text_button']   = Lang::get('quangtrong.news::lang.button.subscribe');
    }
    ...
    public function onSubscription()
    {
        ...
        if ($subscriberResult->count() > 0) {
            ...
            foreach ($data['category'] as $category) {
                if (is_numeric($category) && Categories::where(['id' => $category, 'hidden' => 2])->count() == 1 && Db::table('quangtrong_news_relations')->where(['subscriber_id' => $subscriber->id, 'categories_id' => $data['category']])->count() == 0) {
                    Db::table('quangtrong_news_relations')->insertGetId([
